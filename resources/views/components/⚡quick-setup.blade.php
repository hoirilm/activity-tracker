...
new class extends Component
{
    ...
    public function applyStarterPack()
    {
        ...
        if (!$user->tasks()->exists()) {
            $user->tasks()->create([
                'title' => 'Setup & Inisialisasi Tracker',
                'description' => 'Tugas awal untuk memulai tracking aktivitas harian.',
        ...
        $this->successMessage = 'Starter Pack berhasil diaktifkan! Semua komponen dasar telah siap.';
    ...
    public function createProject()
    {
        ...
        $this->projectName = '';
        $this->projectClient = '';
        $this->successMessage = 'Project berhasil dibuat!';
    ...
    public function createCategory()
    {
        ...
        $this->categoryName = '';
        $this->successMessage = 'Kategori berhasil dibuat!';
    ...
    public function createTask()
    {
        ...
        $this->successMessage = 'Task berhasil dibuat!';
    ...
    public function createLabel()
    {
        ...
        $this->labelName = '';
        $this->successMessage = 'Label berhasil dibuat!';
    ...
    @if(auth()->check() && (!$this->has_project || !$this->has_category) && !$showModal)
        <div class="fixed bottom-4 left-4 z-50 animate-bounce">
            <button @click="$wire.openModal()" type="button" class="flex items-center gap-2.5 px-4 py-2.5 rounded-full bg-gradient-to-r from-amber-500 via-orange-500 to-indigo-600 text-white font-medium text-xs shadow-xl hover:shadow-2xl hover:scale-105 active:scale-95 transition-all duration-200 cursor-pointer border border-amber-300/30">
                ...
                <span>Quick Setup ({{ $this->progress_percent }}%) - Klik untuk melengkapi</span>
            </button>
        </div>
    @endif
                ...
                <div class="bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 p-6 sm:p-8 text-white relative overflow-hidden">
                    ...
                    <div class="flex items-start justify-between gap-4 relative z-10">
                        <div>
                            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/20 backdrop-blur-md text-xs font-semibold text-white mb-3">
                                ...
                                <span>Panduan Pengguna Baru</span>
                            </div>
                            <h2 class="text-xl sm:text-2xl font-bold tracking-tight">Selamat Datang di Activity Tracker! 🚀</h2>
                            <p class="mt-1.5 text-xs sm:text-sm text-indigo-100 max-w-lg leading-relaxed">
                                Mari siapkan data awal Anda. Minimal <strong class="font-extrabold text-white bg-white/20 px-2 py-0.5 rounded-md shadow-2xs">1 Project</strong> dan <strong class="font-extrabold text-white bg-white/20 px-2 py-0.5 rounded-md shadow-2xs">1 Kategori</strong> wajib dibuat agar Time Tracker dapat digunakan.
                            </p>
                        </div>
                    ...
                    <div class="mt-6 pt-4 border-t border-white/20">
                        <div class="flex items-center justify-between text-xs font-semibold mb-2">
                            <span>Progres Setup Awal</span>
                            <span>{{ $this->progress_percent }}% Selesai</span>
                        </div>
                ...
                <div class="p-6 sm:p-8 space-y-6">
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        ...
                        <div class="p-3.5 rounded-2xl border transition-all {{ $this->has_project ? 'bg-emerald-50/50 dark:bg-emerald-950/20 border-emerald-300 dark:border-emerald-800/50' : 'bg-amber-50/50 dark:bg-amber-950/20 border-amber-300 dark:border-amber-800/50' }}">
                            ...
                            <div class="flex items-center gap-1.5 text-xs text-zinc-600 dark:text-zinc-400 font-medium">
                                @if($this->has_project)
                                    ...
                                    <span class="truncate">{{ $this->projects->count() }} dibuat</span>
                                @else
                        ...
                        <div class="p-3.5 rounded-2xl border transition-all {{ $this->has_category ? 'bg-emerald-50/50 dark:bg-emerald-950/20 border-emerald-300 dark:border-emerald-800/50' : 'bg-amber-50/50 dark:bg-amber-950/20 border-amber-300 dark:border-amber-800/50' }}">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-xs font-bold {{ $this->has_category ? 'text-emerald-700 dark:text-emerald-400' : 'text-amber-700 dark:text-amber-400' }}">🏷️ Kategori</span>
                                <span class="px-1.5 py-0.5 text-[10px] font-bold rounded-md {{ $this->has_category ? 'bg-emerald-100 dark:bg-emerald-900/60 text-emerald-800 dark:text-emerald-300' : 'bg-red-100 dark:bg-red-900/60 text-red-700 dark:text-red-300' }}">
                                    {{ $this->has_category ? 'Sudah' : 'Wajib' }}
                                </span>
                            </div>
                            <div class="flex items-center gap-1.5 text-xs text-zinc-600 dark:text-zinc-400 font-medium">
                                @if($this->has_category)
                                    <flux:icon name="check-circle" class="size-4 text-emerald-500 shrink-0" />
                                    <span class="truncate">{{ $this->categories->count() }} dibuat</span>
                                @else
                                    <flux:icon name="exclamation-circle" class="size-4 text-amber-500 shrink-0" />
                                    <span>Belum ada</span>
                                @endif
                        ...
                        <div class="p-3.5 rounded-2xl border transition-all {{ $this->has_task ? 'bg-emerald-50/50 dark:bg-emerald-950/20 border-emerald-300 dark:border-emerald-800/50' : 'bg-zinc-50 dark:bg-zinc-800/40 border-zinc-200 dark:border-zinc-700/60' }}">
                            ...
                            <div class="flex items-center gap-1.5 text-xs text-zinc-600 dark:text-zinc-400 font-medium">
                                @if($this->has_task)
                                    <flux:icon name="check-circle" class="size-4 text-emerald-500 shrink-0" />
                                    <span class="truncate">{{ $this->tasks->count() }} dibuat</span>
                                @else
                        ...
                        <div class="p-3.5 rounded-2xl border transition-all {{ $this->has_label ? 'bg-emerald-50/50 dark:bg-emerald-950/20 border-emerald-300 dark:border-emerald-800/50' : 'bg-zinc-50 dark:bg-zinc-800/40 border-zinc-200 dark:border-zinc-700/60' }}">
                            ...
                            <div class="flex items-center gap-1.5 text-xs text-zinc-600 dark:text-zinc-400 font-medium">
                                @if($this->has_label)
                                    <flux:icon name="check-circle" class="size-4 text-emerald-500 shrink-0" />
                                    <span class="truncate">{{ $this->labels->count() }} dibuat</span>
                                @else
                                    <flux:icon name="minus-circle" class="size-4 text-zinc-400 shrink-0" />
                                    <span>Belum ada</span>
                                @endif
                    ...
                    @if($activeTab === 'starter')
                        <div class="p-5 rounded-2xl bg-gradient-to-br from-indigo-50/80 via-purple-50/40 to-pink-50/50 dark:from-indigo-950/30 dark:via-purple-950/20 dark:to-zinc-900 border border-indigo-200/80 dark:border-indigo-900/50 space-y-4">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <h4 class="text-sm font-bold text-zinc-900 dark:text-white">Opsi Tercepat: Aktifkan Starter Pack Bawaan</h4>
                                    <p class="text-xs text-zinc-600 dark:text-zinc-400 mt-1 leading-relaxed">
                                        Ingin langsung mencoba mencatat aktivitas tanpa ribet? Sistem akan otomatis membuatkan template standar untuk Anda:
                                    </p>
                                    <ul class="mt-2.5 space-y-1 text-xs text-zinc-700 dark:text-zinc-300">
                                        ...
                                        <li class="flex items-center gap-2">
                                            <flux:icon name="check-circle" class="size-4 text-emerald-500 shrink-0" />
                                            <span><strong>Kategori:</strong> Feature Development, Bug Fix & Maintenance, Meeting</span>
                                        </li>
                                        <li class="flex items-center gap-2">
                                            <flux:icon name="check-circle" class="size-4 text-emerald-500 shrink-0" />
                                            <span><strong>Task:</strong> Setup & Inisialisasi Tracker</span>
                                        </li>
                                        <li class="flex items-center gap-2">
                                            <flux:icon name="check-circle" class="size-4 text-emerald-500 shrink-0" />
                            ...
                            <div class="pt-2 flex items-center justify-end">
                                <button wire:click="applyStarterPack" wire:loading.attr="disabled" type="button" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-semibold text-xs shadow-lg hover:shadow-xl active:scale-95 transition-all cursor-pointer">
                                    ...
                                    <span>Aktifkan Starter Pack Sekarang</span>
                                </button>
                            </div>
                        </div>
                    @endif
                    ...
                    @if($activeTab === 'project')
                        <div class="space-y-4">
                            <form wire:submit.prevent="createProject" class="space-y-3 bg-zinc-50 dark:bg-zinc-800/50 p-4 rounded-2xl border border-zinc-200/80 dark:border-zinc-700/60">
                                <h4 class="text-xs font-bold text-zinc-900 dark:text-white uppercase tracking-wider">Tambah Project Baru</h4>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <div>
                                        <label class="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1">Nama Project <span class="text-red-500">*</span></label>
                                        <input wire:model="projectName" type="text" placeholder="Contoh: Website Revamp" class="w-full px-3 py-2 text-xs rounded-xl border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:outline-hidden" />
                                        @error('projectName') <span class="text-[10px] text-red-500 mt-0.5 block">{{ $message }}</span> @enderror
                                    </div>
                                    <div>
                                        <label class="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1">Nama Klien (Opsional)</label>
                                        <input wire:model="projectClient" type="text" placeholder="Contoh: PT ABC" class="w-full px-3 py-2 text-xs rounded-xl border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:outline-hidden" />
                                    </div>
                                </div>
                            ...
                            <div>
                                <h5 class="text-xs font-semibold text-zinc-600 dark:text-zinc-400 mb-2">Daftar Project Anda ({{ $this->projects->count() }}):</h5>
                                @if($this->has_project)
                                ...
                                @else
                                    <p class="text-xs text-zinc-400 italic">Belum ada project yang dibuat.</p>
                                @endif
                            </div>
                        </div>
                    @endif
                    ...
                    @if($activeTab === 'category')
                        <div class="space-y-4">
                            <form wire:submit.prevent="createCategory" class="space-y-3 bg-zinc-50 dark:bg-zinc-800/50 p-4 rounded-2xl border border-zinc-200/80 dark:border-zinc-700/60">
                                <h4 class="text-xs font-bold text-zinc-900 dark:text-white uppercase tracking-wider">Tambah Kategori Baru</h4>
                                <div>
                                    <label class="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1">Nama Kategori <span class="text-red-500">*</span></label>
                                    <input wire:model="categoryName" type="text" placeholder="Contoh: Development, Design, Meeting" class="w-full px-3 py-2 text-xs rounded-xl border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:outline-hidden" />
                            ...
                            <div>
                                <h5 class="text-xs font-semibold text-zinc-600 dark:text-zinc-400 mb-2">Daftar Kategori Anda ({{ $this->categories->count() }}):</h5>
                                @if($this->has_category)
                                    <div class="flex flex-wrap gap-2">
                                ...
                                @else
                                    <p class="text-xs text-zinc-400 italic">Belum ada kategori yang dibuat.</p>
                                @endif
                            </div>
                        </div>
                    @endif
                    ...
                    @if($activeTab === 'task')
                        <div class="space-y-4">
                            <form wire:submit.prevent="createTask" class="space-y-3 bg-zinc-50 dark:bg-zinc-800/50 p-4 rounded-2xl border border-zinc-200/80 dark:border-zinc-700/60">
                                <h4 class="text-xs font-bold text-zinc-900 dark:text-white uppercase tracking-wider">Tambah Task Baru (Opsional)</h4>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <div>
                                        <label class="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1">Judul Task <span class="text-red-500">*</span></label>
                                        <input wire:model="taskTitle" type="text" placeholder="Contoh: Slice UI Landing Page" class="w-full px-3 py-2 text-xs rounded-xl border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:outline-hidden" />
                                        @error('taskTitle') <span class="text-[10px] text-red-500 mt-0.5 block">{{ $message }}</span> @enderror
                                    </div>
                                    <div>
                                        <label class="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1">Pilih Project</label>
                                ...
                                <div>
                                    <label class="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1">Deskripsi (Opsional)</label>
                                    <textarea wire:model="taskDescription" rows="2" placeholder="Catatan detail tugas..." class="w-full px-3 py-2 text-xs rounded-xl border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:outline-hidden"></textarea>
                                </div>
                                <div class="flex justify-end pt-1">
                                    <button type="submit" class="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-medium text-xs shadow-md transition-all cursor-pointer">
                                        + Simpan Task
                            ...
                            <!-- List existing tasks -->
                            <div>
                                <h5 class="text-xs font-semibold text-zinc-600 dark:text-zinc-400 mb-2">Daftar Task Anda ({{ $this->tasks->count() }}):</h5>
                                @if($this->has_task)
                                ...
                                @else
                                    <p class="text-xs text-zinc-400 italic">Belum ada task yang dibuat.</p>
                                @endif
                            </div>
                        </div>
                    @endif
                    ...
                    @if($activeTab === 'label')
                        <div class="space-y-4">
                            <form wire:submit.prevent="createLabel" class="space-y-3 bg-zinc-50 dark:bg-zinc-800/50 p-4 rounded-2xl border border-zinc-200/80 dark:border-zinc-700/60">
                                <h4 class="text-xs font-bold text-zinc-900 dark:text-white uppercase tracking-wider">Tambah Label Baru (Opsional)</h4>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <div>
                                        <label class="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1">Nama Label <span class="text-red-500">*</span></label>
                                        <input wire:model="labelName" type="text" placeholder="Contoh: Urgent, Frontend" class="w-full px-3 py-2 text-xs rounded-xl border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:outline-hidden" />
                                        @error('labelName') <span class="text-[10px] text-red-500 mt-0.5 block">{{ $message }}</span> @enderror
                                    </div>
                                    <div>
                                        <label class="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1">Warna Label</label>
                            ...
                            <div>
                                <h5 class="text-xs font-semibold text-zinc-600 dark:text-zinc-400 mb-2">Daftar Label Anda ({{ $this->labels->count() }}):</h5>
                                @if($this->has_label)
                                    <div class="flex flex-wrap gap-2">
                                ...
                                @else
                                    <p class="text-xs text-zinc-400 italic">Belum ada label yang dibuat.</p>
                                @endif
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="text-xs text-zinc-600 dark:text-zinc-400">
                        @if($this->has_project && $this->has_category)
                            <span class="text-emerald-600 dark:text-emerald-400 font-semibold flex items-center gap-1">
                                <flux:icon name="check-circle" class="size-4 shrink-0 text-emerald-500" />
                                <span>Syarat wajib terpenuhi! Anda siap menggunakan Tracker.</span>
                            </span>
                        @else
                            <span class="text-amber-600 dark:text-amber-400 font-medium flex items-center gap-1">
                                ...
                                <span>Buat minimal 1 Project & 1 Kategori untuk menyelesaikan setup.</span>
                            </span>
                        @endif
                    </div>
