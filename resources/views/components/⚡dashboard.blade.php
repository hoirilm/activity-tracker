...
<div class="relative flex min-h-[calc(100vh-10rem)] w-full flex-col gap-5 sm:gap-6 px-3 sm:px-4 py-2 text-neutral-900 dark:text-neutral-100 max-w-6xl mx-auto mt-1 sm:mt-4 pb-12 overflow-hidden" x-data="{ scrolled: false }" @scroll.window="scrolled = (window.pageYOffset > 300)">

    <!-- Background Ambient Warm Glow Orbs -->
    <div class="absolute -top-20 -left-20 w-80 h-80 rounded-full bg-amber-500/10 blur-3xl pointer-events-none"></div>
    <div class="absolute top-1/3 -right-20 w-80 h-80 rounded-full bg-orange-500/10 blur-3xl pointer-events-none"></div>

    <!-- Main Animated Content -->
    <div class="relative z-10 flex flex-col gap-6 animate-page-entrance">
        <!-- Header -->
        <div class="border-b border-zinc-200/80 dark:border-zinc-800/80 pb-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h2 class="text-xl sm:text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100 flex items-center gap-2.5">
                    <div class="size-9 rounded-xl bg-amber-500/10 dark:bg-amber-500/15 border border-amber-500/25 flex items-center justify-center text-amber-500 dark:text-amber-400 shrink-0 shadow-xs">
                        <flux:icon name="chart-bar" class="size-4.5 text-amber-500 dark:text-amber-400" />
                    </div>
                    <span>Dashboard Overview</span>
                </h2>
                <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-1">Welcome back, <span class="text-zinc-900 dark:text-zinc-200 font-semibold">{{ auth()->user()->name }}</span>! Here is your real-time activity metrics breakdown.</p>
            </div>

            <div class="flex items-center gap-2">
                <span class="text-[11px] font-mono font-semibold px-3 py-1 rounded-xl bg-amber-500/10 dark:bg-amber-500/15 text-amber-700 dark:text-amber-300 border border-amber-500/25 flex items-center gap-2 shadow-2xs backdrop-blur-md">
        ...
        <div class="grid grid-cols-3 gap-2 sm:gap-5">
            ...
            <div class="relative overflow-hidden bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-zinc-200/80 dark:border-zinc-800/80 rounded-xl sm:rounded-2xl p-2.5 sm:p-5 shadow-xs flex flex-col justify-between group hover:border-amber-500/50 hover:shadow-lg hover:shadow-amber-500/5 transition-all duration-300">
                ...
                <div class="flex items-center justify-between mb-1.5 sm:mb-3">
                    <div class="size-7 sm:size-10 rounded-lg sm:rounded-xl bg-amber-500/10 dark:bg-amber-500/15 border border-amber-500/20 dark:border-amber-500/30 flex items-center justify-center text-amber-500 dark:text-amber-400 shrink-0 group-hover:scale-105 transition-transform">
                        <flux:icon name="clock" class="size-3.5 sm:size-5 text-amber-500 dark:text-amber-400" />
                    </div>
                    <span class="bg-amber-500/10 text-amber-700 dark:text-amber-400 text-[8px] sm:text-[10px] font-mono font-bold px-1.5 sm:px-2 py-0.5 rounded-md border border-amber-500/20">
                        Today
                    </span>
                </div>
                ...
                <div>
                    ...
                    <div class="text-base sm:text-3xl font-mono font-extrabold tracking-tight text-amber-600 dark:text-amber-400 mt-0.5 sm:mt-1 truncate">{{ $this->todayDuration }}</div>
            ...
            <!-- Card 2: This Week -->
            <div class="relative overflow-hidden bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-zinc-200/80 dark:border-zinc-800/80 rounded-xl sm:rounded-2xl p-2.5 sm:p-5 shadow-xs flex flex-col justify-between group hover:border-orange-500/50 hover:shadow-lg hover:shadow-orange-500/5 transition-all duration-300">
                <div class="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-orange-500/50 to-transparent"></div>
                <div class="flex items-center justify-between mb-1.5 sm:mb-3">
                    <div class="size-7 sm:size-10 rounded-lg sm:rounded-xl bg-orange-500/10 dark:bg-orange-500/15 border border-orange-500/20 dark:border-orange-500/30 flex items-center justify-center text-orange-500 dark:text-orange-400 shrink-0 group-hover:scale-105 transition-transform">
                        <flux:icon name="calendar-days" class="size-3.5 sm:size-5 text-orange-500 dark:text-orange-400" />
                    </div>
                    <span class="bg-orange-500/10 text-orange-700 dark:text-orange-400 text-[8px] sm:text-[10px] font-mono font-bold px-1.5 sm:px-2 py-0.5 rounded-md border border-orange-500/20">
                        Week
                    </span>
                </div>
            ...
            <div class="relative overflow-hidden bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-zinc-200/80 dark:border-zinc-800/80 rounded-xl sm:rounded-2xl p-2.5 sm:p-5 shadow-xs flex flex-col justify-between group hover:border-amber-500/50 hover:shadow-lg hover:shadow-amber-500/5 transition-all duration-300">
                ...
                <div class="flex items-center justify-between mb-1.5 sm:mb-3">
                    <div class="size-7 sm:size-10 rounded-lg sm:rounded-xl bg-amber-500/10 dark:bg-amber-500/15 border border-amber-500/20 dark:border-amber-500/30 flex items-center justify-center text-amber-500 dark:text-amber-400 shrink-0 group-hover:scale-105 transition-transform">
                        <flux:icon name="briefcase" class="size-3.5 sm:size-5 text-amber-500 dark:text-amber-400" />
                    </div>
                    <span class="bg-amber-500/10 text-amber-700 dark:text-amber-400 text-[8px] sm:text-[10px] font-mono font-bold px-1.5 sm:px-2 py-0.5 rounded-md border border-amber-500/20">
                        Active
                    </span>
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <div class="text-xs text-neutral-500 dark:text-neutral-400 mt-1 flex flex-wrap items-center gap-2">
                                ...
                                @if($running->is_parallel) 
                                    <span class="bg-amber-500/10 text-amber-600 dark:text-amber-400 text-[9px] uppercase tracking-wider px-2 py-0.5 rounded-md font-mono font-bold ml-1 border border-amber-500/20">Parallel</span> 
                                @endif
                            </div>
                        </div>
        ...
        <div class="flex flex-col gap-3.5 mt-2 bg-white/70 dark:bg-zinc-900/80 backdrop-blur-xl border border-zinc-200/80 dark:border-zinc-800 rounded-2xl p-4 shadow-sm hover:border-amber-500/30 transition-all duration-300 relative overflow-hidden">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-zinc-200/60 dark:border-zinc-800/60">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2 mt-1">
                            <div class="w-28 sm:w-36 bg-zinc-200 dark:bg-zinc-800 h-1.5 rounded-full overflow-hidden">
                                <div class="bg-gradient-to-r from-amber-500 via-orange-400 to-emerald-500 h-full transition-all duration-500 rounded-full" style="width: {{ $this->taskProgressStats['percentage'] }}%"></div>
                            </div>
                ...
                <div class="flex items-center gap-2 self-start sm:self-auto">
                    <button type="button" 
                            ...
                            class="px-3 py-1.5 rounded-xl text-xs font-bold bg-gradient-to-r from-amber-400 via-amber-500 to-orange-500 hover:from-amber-300 hover:to-orange-400 text-zinc-950 transition-all flex items-center gap-1.5 cursor-pointer active:scale-95 shadow-md shadow-amber-500/20">
                        <flux:icon name="plus" class="size-3.5 stroke-[3]" />
                        <span>Quick Add</span>
                    </button>

                    <flux:button variant="subtle" size="xs" href="{{ route('manage') }}" wire:navigate class="cursor-pointer font-semibold text-xs text-zinc-500 hover:text-amber-500 dark:text-zinc-400 dark:hover:text-amber-400 active:scale-95 transition-all flex items-center gap-1">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                ...
                <div class="flex items-center gap-1 bg-zinc-100 dark:bg-zinc-950/70 p-1 rounded-xl border border-zinc-200/80 dark:border-zinc-800/80 overflow-x-auto custom-scrollbar">
                    <button type="button" wire:click="$set('taskFilter', 'on_progress')"
                            class="px-2.5 py-1 rounded-lg text-xs font-mono font-bold transition-all whitespace-nowrap cursor-pointer flex items-center gap-1.5 {{ $taskFilter === 'on_progress' ? 'bg-white dark:bg-zinc-800 text-amber-600 dark:text-amber-400 shadow-2xs border border-amber-500/30' : 'text-zinc-500 hover:text-zinc-800 dark:text-zinc-400 dark:hover:text-zinc-200' }}">
                        <span class="size-1.5 rounded-full bg-amber-500"></span>
                    ...
                    <button type="button" wire:click="$set('taskFilter', 'done_today')"
                            class="px-2.5 py-1 rounded-lg text-xs font-mono font-bold transition-all whitespace-nowrap cursor-pointer flex items-center gap-1.5 {{ $taskFilter === 'done_today' ? 'bg-white dark:bg-zinc-800 text-emerald-600 dark:text-emerald-400 shadow-2xs border border-emerald-500/30' : 'text-zinc-500 hover:text-zinc-800 dark:text-zinc-400 dark:hover:text-zinc-200' }}">
                ...
                <div class="relative w-full sm:w-52">
                    ...
                    <input type="text" 
                           ...
                           class="w-full h-8 pl-8 pr-7 rounded-xl text-xs bg-zinc-100 dark:bg-zinc-950/70 border border-zinc-200/80 dark:border-zinc-800 text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 focus:outline-none focus:border-amber-500 transition-all" />
                    @if($taskSearch)
            ...
            @if($showQuickTaskForm)
                <form wire:submit.prevent="createQuickTask" class="p-3 bg-amber-500/5 dark:bg-amber-500/10 border border-amber-500/20 rounded-xl flex flex-col sm:flex-row gap-2 items-stretch sm:items-center">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button type="submit" class="h-9 px-3.5 bg-gradient-to-r from-amber-400 via-amber-500 to-orange-500 hover:from-amber-300 hover:to-orange-400 text-zinc-950 font-bold text-xs rounded-lg transition-all active:scale-95 shadow-md shadow-amber-500/20 flex items-center gap-1 cursor-pointer shrink-0">
                            <flux:icon name="plus" class="size-3.5 stroke-[3]" />
                            <span>Add Task</span>
                        </button>
                ...
                <div class="divide-y divide-zinc-200/50 dark:divide-zinc-800/50 border border-zinc-200/80 dark:border-zinc-800 rounded-xl overflow-hidden bg-white/40 dark:bg-zinc-900/40">
                    @foreach($this->filteredTasks as $task)
                        <div wire:key="dashboard-stream-task-{{ $task->id }}" 
                             ...
                             class="group px-3.5 py-3 hover:bg-amber-500/5 dark:hover:bg-amber-500/5 transition-all duration-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            ...
                            <div class="min-w-0 flex-1 flex flex-col sm:flex-row sm:items-center gap-2">
                                ...
                                @else
                                    <button type="button" 
                                            ...
                                            class="size-5 rounded-md border border-zinc-300 dark:border-zinc-600 hover:border-amber-500 group-hover:border-amber-500 flex items-center justify-center shrink-0 transition-all cursor-pointer active:scale-95"
                                            title="Mark as Done">
                                    ...
                                    <div class="flex items-center gap-1.5 shrink-0 flex-wrap">
                                        ...
                                        @if($task->due_badge)
                                            ...
                                            <span class="inline-flex items-center gap-1 text-[9px] font-semibold px-2 py-0.5 rounded-md border
                                                ...
                                                {{ $badge['color'] === 'indigo' ? 'bg-amber-500/10 text-amber-700 dark:text-amber-300 border-amber-200/80 dark:border-amber-900/40' : '' }}
            ...
            @elseif($taskFilter === 'on_progress')
                ...
                <div class="py-10 px-6 rounded-2xl text-center border border-dashed border-amber-500/25 bg-gradient-to-b from-amber-500/5 via-transparent to-emerald-500/5 flex flex-col items-center justify-center gap-3">
                    ...
                    <button type="button" 
                            ...
                            class="mt-2 px-4 py-2 text-xs font-bold bg-gradient-to-r from-amber-400 via-amber-500 to-orange-500 hover:from-amber-300 hover:to-orange-400 text-zinc-950 rounded-xl shadow-md shadow-amber-500/20 transition-all active:scale-95 flex items-center gap-1.5 cursor-pointer">
                        <flux:icon name="plus" class="size-3.5 stroke-[3]" />
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <!-- Activity Chart -->
            <div wire:ignore class="lg:col-span-2 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-zinc-200/80 dark:border-zinc-800 rounded-2xl p-5 shadow-xs relative overflow-hidden flex flex-col justify-between group hover:border-amber-500/40 hover:shadow-lg hover:shadow-amber-500/5 transition-all duration-300"
                 x-data="{
                     period: 'weekly',
                     ...
                     initChart(labels, data) {
                         ...
                         const textColor = this.isDark ? '#a1a1aa' : '#71717a';
                         ...
                         this.$refs.canvas.chartInstance = new Chart(ctx, {
                             ...
                             data: {
                                 ...
                                 datasets: [{
                                     label: 'Hours Worked',
                                     data: data,
                                     borderColor: this.isDark ? '#fbbf24' : '#f59e0b',
                                     backgroundColor: gradient,
                                     borderWidth: 2.5,
                                     fill: true,
                                     tension: 0.4,
                                     pointBackgroundColor: this.isDark ? '#09090b' : '#ffffff',
                                     pointBorderColor: this.isDark ? '#fbbf24' : '#f59e0b',
                                     pointBorderWidth: 2,
                                     pointRadius: 0,
                                     pointHoverRadius: 5,
                                     pointHoverBackgroundColor: this.isDark ? '#fbbf24' : '#f59e0b',
                             ...
                             options: {
                                 ...
                                 plugins: {
                                     ...
                                     tooltip: {
                                         backgroundColor: this.isDark ? 'rgba(9, 9, 11, 0.95)' : 'rgba(255, 255, 255, 0.95)',
                                         titleColor: this.isDark ? '#f4f4f5' : '#18181b',
                                         bodyColor: this.isDark ? '#d4d4d8' : '#52525b',
                                         borderColor: this.isDark ? '#f59e0b40' : '#e4e4e7',
                                         borderWidth: 1,
                                         padding: 12,
                                         displayColors: false,
                ...
                <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-4 relative z-10">
                    <h3 class="text-sm font-bold text-zinc-900 dark:text-zinc-100 flex items-center gap-2">
                        <div class="size-7 rounded-lg bg-amber-500/10 dark:bg-amber-500/15 border border-amber-500/20 flex items-center justify-center text-amber-500 dark:text-amber-400 shrink-0">
                            <flux:icon name="chart-bar" class="size-4 text-amber-500 dark:text-amber-400" />
                        </div>
                        <span>Activity Overview (Hours)</span>
                    </h3>
                    
                    <div class="flex bg-zinc-100 dark:bg-zinc-950/80 rounded-xl p-1 self-start sm:self-auto shrink-0 border border-zinc-200/80 dark:border-zinc-800/80">
                        <button type="button" @click="period = 'weekly'; $wire.set('chartPeriod', 'weekly')" 
                                class="text-[10px] px-3 py-1.5 rounded-lg font-bold uppercase tracking-wider transition-all duration-200 cursor-pointer"
                                :class="period === 'weekly' ? 'bg-gradient-to-r from-amber-400 to-orange-400 text-zinc-950 shadow-xs' : 'text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-200'">
                            Weekly
                        </button>
                        <button type="button" @click="period = 'monthly'; $wire.set('chartPeriod', 'monthly')" 
                                class="text-[10px] px-3 py-1.5 rounded-lg font-bold uppercase tracking-wider transition-all duration-200 cursor-pointer"
                                :class="period === 'monthly' ? 'bg-gradient-to-r from-amber-400 to-orange-400 text-zinc-950 shadow-xs' : 'text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-200'">
                            Monthly
                        </button>
                    </div>
                </div>
            ...
            <div class="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-zinc-200/80 dark:border-zinc-800 rounded-2xl p-5 shadow-xs flex flex-col justify-between relative overflow-hidden group hover:border-amber-500/40 hover:shadow-lg hover:shadow-amber-500/5 transition-all duration-300">
                <div class="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-amber-500/40 to-transparent"></div>
                <div>
                    <div class="flex items-center justify-between mb-4 shrink-0 relative z-10">
                        <h3 class="text-sm font-bold text-zinc-900 dark:text-zinc-100 flex items-center gap-2">
                            <div class="size-7 rounded-lg bg-amber-500/10 dark:bg-amber-500/15 border border-amber-500/20 flex items-center justify-center text-amber-500 dark:text-amber-400 shrink-0">
                                <flux:icon name="chart-pie" class="size-4 text-amber-500 dark:text-amber-400" />
                            </div>
                            <span>Time Allocation</span>
                        </h3>
                        <span class="text-[10px] font-mono font-bold text-amber-600 dark:text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded-md border border-amber-500/20">Weekly</span>
                    </div>
                                ...
                                <div x-data="{ open: false }" class="space-y-2">
                                    <button type="button" @click="open = !open" class="w-full text-left focus:outline-none group cursor-pointer block">
                                        <div class="flex items-center justify-between text-xs mb-1.5">
                                            <div class="flex items-center gap-1.5 font-semibold text-zinc-800 dark:text-zinc-200 group-hover:text-amber-500 transition-colors">
                                        ...
                                        <div class="w-full bg-zinc-100 dark:bg-zinc-950 rounded-full h-2 overflow-hidden p-0.5 border border-zinc-200/50 dark:border-zinc-800">
                                            <div class="bg-gradient-to-r from-amber-400 via-amber-500 to-orange-500 h-1.5 rounded-full transition-all duration-500 shadow-2xs" style="width: {{ $stat['percentage'] }}%"></div>
                                        </div>
                                    </button>

                                    <div x-show="open" 
                                         x-collapse
                                         class="pl-4 pr-1 space-y-2 border-l-2 border-amber-500/40 dark:border-amber-500/30 ml-2 mt-2" 
                                         style="display: none;">
                    ...
                    @else
                        <div class="flex-1 flex flex-col items-center justify-center text-center text-neutral-500 dark:text-neutral-400 space-y-2.5 py-8 border border-dashed border-zinc-200/80 dark:border-zinc-800 rounded-xl bg-zinc-50/50 dark:bg-zinc-950/30 relative z-10">
                            <div class="size-10 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-500 dark:text-amber-400">
        ...
        <div class="flex flex-col gap-3 mt-2">
            <div class="flex items-center justify-between">
                ...
                <flux:button variant="subtle" size="xs" href="{{ route('tracker') }}" wire:navigate class="cursor-pointer font-bold text-amber-600 dark:text-amber-400 hover:text-amber-500 active:scale-95 transition-transform flex items-center gap-1">
                    <span>View All Tracker</span>
                    <flux:icon name="arrow-right" class="size-3" />
                </flux:button>
            </div>
            
            <div class="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-zinc-200/80 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-xs divide-y divide-zinc-200/50 dark:divide-zinc-800/50 relative group hover:border-amber-500/40 hover:shadow-lg hover:shadow-amber-500/5 transition-all duration-300">
                <div class="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-amber-500/40 to-transparent"></div>
                @forelse($this->recentActivities as $activity)
                    <div class="px-4 py-3.5 hover:bg-amber-500/5 dark:hover:bg-amber-500/5 transition-colors flex items-center justify-between gap-3 group/row relative z-10">
                        <!-- Left active hover indicator line -->
                        <div class="absolute left-0 top-0 bottom-0 w-1 bg-amber-500 opacity-0 group-hover/row:opacity-100 transition-opacity"></div>
                        
                        <div class="flex items-center gap-3.5 min-w-0">
                            <div class="size-9 rounded-xl bg-amber-500/10 dark:bg-amber-500/15 border border-amber-500/20 flex items-center justify-center text-amber-500 dark:text-amber-400 shrink-0 group-hover/row:scale-105 transition-transform">
                                <flux:icon name="folder" class="size-4 text-amber-500 dark:text-amber-400" />
                            </div>
                            <div class="min-w-0">
                                <div class="font-bold text-sm text-zinc-900 dark:text-zinc-100 truncate group-hover/row:text-amber-600 dark:group-hover/row:text-amber-400 transition-colors">{{ $activity->detail }}</div>
                                <div class="text-[11px] truncate flex items-center gap-1.5 mt-1 flex-wrap">
                                    @if($activity->task)
                                        <span class="bg-amber-500/10 text-amber-700 dark:text-amber-400 text-[10px] font-semibold px-2 py-0.5 rounded-md border border-amber-500/20 flex items-center gap-1 shrink-0">
                                    ...
                                    @if($activity->is_parallel) 
                                        <span class="bg-amber-500/10 text-amber-600 dark:text-amber-400 text-[9px] font-mono font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-md border border-amber-500/20 shrink-0">Parallel</span>
                                    @endif
                                </div>
                            </div>
                        </div>
                        <div class="shrink-0 text-right">
                            <div class="font-mono text-sm font-extrabold text-amber-600 dark:text-amber-400 tracking-tight">{{ $activity->duration }}</div>
