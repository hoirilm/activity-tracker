...
<div class="flex h-full w-full flex-col gap-5 md:gap-6 px-3 sm:px-4 py-2 text-neutral-900 dark:text-neutral-100 max-w-6xl mx-auto mt-0 md:mt-4 pb-32 md:pb-8" x-data="{ scrolled: false }" @scroll.window="scrolled = (window.pageYOffset > 300)">
    ...
    <!-- Running / Paused Activities Section -->
                    ...
                    <div class="divide-y divide-zinc-200/40 dark:divide-zinc-800/40">
                        @foreach($dayActivities as $activity)
                            <div wire:key="activity-{{ $activity->id }}" class="group px-4 py-3.5 hover:bg-zinc-50 dark:hover:bg-zinc-800/40 transition-colors flex items-center justify-between gap-3 relative overflow-hidden">
                                ...
                                <!-- Right: Duration & Time Range Indicator & Dropdown -->
                                <div class="flex items-center gap-3 shrink-0">
                                    <div class="flex flex-col items-end gap-1">
                                        ...
                                        <div class="font-mono text-[10px] text-zinc-500 dark:text-zinc-400 inline-flex items-center gap-1.5">
    ...
    <flux:modal name="delete-activity-modal" class="w-[calc(100vw-2rem)] max-w-md backdrop:backdrop-blur-md z-[200]">
        <div x-data="{ id: null, detail: '' }" 
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <flux:heading size="lg" class="font-bold">Delete Activity?</flux:heading>
                    <flux:text class="mt-0.5 text-xs text-zinc-500 dark:text-zinc-400">
                        Are you sure you want to delete <strong class="text-zinc-900 dark:text-zinc-100" x-text="detail ? '&ldquo;' + detail + '&rdquo;' : 'this activity'"></strong>?
                    </flux:text>
                </div>
            </div>
            <div class="flex justify-end gap-2 pt-2">
                ...
                <flux:modal.close>
                    <flux:button variant="danger" size="sm" class="cursor-pointer" @click="$wire.deleteActivity(id)">Delete</flux:button>
                </flux:modal.close>
            </div>
        </div>
    </flux:modal>
