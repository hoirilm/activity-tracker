...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 antialiased selection:bg-amber-500 selection:text-white" x-data="{ mobileNavOpen: false }">
        <!-- Desktop Sidebar (Hidden on mobile) -->
        <flux:sidebar sticky collapsible class="hidden lg:flex h-screen border-e border-zinc-200/80 bg-white/80 dark:border-zinc-800/80 dark:bg-zinc-950/90 backdrop-blur-xl">
            <flux:sidebar.header>
        ...
        <div x-show="mobileNavOpen"
            ...
            <div class="space-y-6">
                ...
                <nav class="space-y-1">
                    ...
                    <a href="https://github.com/hoirilm/activity-tracker" target="_blank" class="flex items-center gap-3 px-3.5 py-2 rounded-xl text-sm font-medium text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-900 hover:text-zinc-900 dark:hover:text-zinc-200 transition-colors">
                        <flux:icon name="folder-git-2" class="size-5" />
                        <span>{{ __('Repository') }}</span>
                    </a>

                    <a href="https://laravel.com/docs/starter-kits#livewire" target="_blank" class="flex items-center gap-3 px-3.5 py-2 rounded-xl text-sm font-medium text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-900 hover:text-zinc-900 dark:hover:text-zinc-200 transition-colors">
        ...
        <div x-data="{ open: false }" @click.outside="open = false" class="fixed top-3.5 right-[3.4rem] sm:top-4 sm:right-[3.65rem] z-40">
            ...
            <div x-show="open" 
                 ...
                 class="absolute top-12 right-0 mt-2 w-52 rounded-2xl border border-zinc-200/80 dark:border-zinc-800 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-xl shadow-xl py-1.5 z-50 origin-top-right overflow-hidden"
                 style="display: none;">
                
                <div class="px-3 py-1.5 text-[10px] font-semibold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider">Help & Support</div>
                
                <!-- Quick Setup -->
                <button @click="open = false; $dispatch('open-quick-setup')" class="w-full flex items-center gap-2.5 px-3 py-2 text-xs font-bold text-amber-600 dark:text-amber-400 hover:bg-amber-500/10 dark:hover:bg-amber-500/15 transition-colors duration-150 text-left cursor-pointer">
        ...
        <div x-data="{ scrolled: false }" 
            ...
            <button 
                ...
                type="button"
                class="pointer-events-auto flex items-center gap-2 px-4 py-2.5 rounded-full bg-zinc-900/90 dark:bg-zinc-900/90 text-white dark:text-zinc-100 hover:text-amber-400 dark:hover:text-amber-400 backdrop-blur-xl shadow-xl hover:shadow-amber-500/10 hover:scale-105 active:scale-95 transition-all duration-200 cursor-pointer border border-zinc-700/60 hover:border-amber-500/40 text-xs font-semibold"
                title="Back to top"
            >
