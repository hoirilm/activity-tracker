<x-layouts::auth :title="__('Log in')">
    <div class="w-full max-w-5xl overflow-hidden rounded-3xl border border-zinc-800/80 bg-zinc-900/60 backdrop-blur-2xl shadow-2xl grid grid-cols-1 lg:grid-cols-12 min-h-[620px]">
            ...
            <div class="relative z-10">
                ...
                <div class="mt-12 space-y-4">
                    ...
                    <h1 class="text-3xl sm:text-4xl font-extrabold tracking-tight text-white leading-tight">
                        Streamline your <span class="bg-gradient-to-r from-emerald-400 via-teal-300 to-indigo-400 bg-clip-text text-transparent">developer workflow</span>.
                    </h1>

                    <p class="text-zinc-400 text-sm leading-relaxed">
                        Track tasks in real-time, maintain deep work without context switching, and seamlessly manage parallel development activities.
                    </p>
                </div>
            </div>
        ...
        <div class="lg:col-span-6 p-8 sm:p-12 flex flex-col justify-center bg-zinc-900/40">
            <div class="w-full max-w-md mx-auto space-y-6">
                ...
                <div class="space-y-1 text-left">
                    <h2 class="text-2xl font-bold tracking-tight text-white">{{ __('Welcome back') }}</h2>
                    <p class="text-sm text-zinc-400">{{ __('Enter your credentials to access your activity tracker') }}</p>
                </div>
                ...
                <!-- Primary Login Form -->
                <form method="POST" action="{{ route('login.store') }}" class="space-y-5" autocomplete="off">
                    @csrf
                    ...
                    <div class="space-y-1.5 relative">
                        ...
                        @if (Route::has('password.request'))
                            <flux:link class="absolute top-0 text-xs text-indigo-400 hover:text-indigo-300 end-0 font-medium transition-colors" :href="route('password.request')" wire:navigate>
                                {{ __('Forgot password?') }}
                            </flux:link>
                        @endif
                    </div>
                    ...
                    <div class="relative flex items-center py-2">
                        ...
                        <span class="flex-shrink-0 mx-4 text-[11px] font-semibold uppercase tracking-wider text-zinc-500">{{ __('Or continue with') }}</span>
                        <div class="flex-grow border-t border-zinc-800"></div>
                    </div>
                ...
                <div class="text-center text-xs text-zinc-400 pt-2">
                    ...
                    <flux:link :href="route('register')" wire:navigate class="text-indigo-400 hover:text-indigo-300 font-medium ml-1">{{ __('Sign up') }}</flux:link>
                </div>
            </div>
        </div>
